...
public:
    charPos() {}
    charPos(int v,int ix, int iy, int w, int h) { value = v; x = ix; y = iy; xe= x + w; ye = y + h;}
    isHit(int ix, int iy) { return ix >= x && ix < xe && iy >= y && iy < ye ? 1 : 0; }
private:
    int value;
    int x;
    int y;
...
{
public:
    int getCharCount() { return len; }
private:
    vector<charPos> posline;
    int xs;
    int ys;
    int xe;
    int height;
...
public:
	...
	void 		loadFont(string filename, int fontsize, bool _bAntiAliased, bool _bFullCharacterSet, bool makeContours = false, bool contourThickness = 0);
    ...
    float         getLineGap() {return lineHeight - fontSize;}
	
	charPos*		drawString(string s, float x, float y);
	int 		nCharacters;
	
	ofTTFCharacter getCharacterAsPoints(int character);
    int         parseColor(const string &c,int index);
    void        resetColor();
...
protected:
	...
	int				fontSize;
    float           linegapscaling;
	bool			bMakeContours;

    charPos         *cpos;
    
#ifdef FTGL
    int             drawChar(int c, float x, float y, int kerning);
#else
    void 			drawChar(int c, float x, float y, int kerning);
#endif
